 ...
 * @since 1.0.4
 */
public class LinePaintScaleLegend extends Title implements PublicCloneable,AxisChangeListener {
	 ...
	 */
	public LinePaintScaleLegend(PaintScale scale, ValueAxis axis) {
		if (axis == null) {
	...
	public Object draw(Graphics2D g2, Rectangle2D area, Object params) {
		...
		if (RectangleEdge.isTopOrBottom(getPosition())) {
			...
			double leftX = target.getMinX();
			int lineCount = (int)Math.rint(target.getWidth());
			//int blockWidth = (int)Math.rint(target.getWidth() / subdivisions);
			//if (blockWidth == 0) blockWidth = 1;
			int blockWidth = 1;
			double topY = 0;
			Rectangle2D outlineRect = new Rectangle2D.Double(0.0,0.0,0.0,0.0);
			double axisCursor = 0.0;
			if (axisEdge == RectangleEdge.TOP) {
				topY = target.getMaxY() - this.stripWidth;
				outlineRect = new Rectangle2D.Double(target.getMinX(), 
						target.getMaxY() - this.stripWidth, target.getWidth(), 
						this.stripWidth);
				axisCursor = target.getMaxY() - this.stripWidth - this.axisOffset;
			}
			else if (axisEdge == RectangleEdge.BOTTOM) {
				topY = target.getMinY();
				outlineRect = new Rectangle2D.Double(target.getMinX(),
						target.getMinY(), target.getWidth(), this.stripWidth);
				axisCursor = target.getMinY() + this.stripWidth + this.axisOffset;
			}

			for(int i = 0; i < lineCount; i = i + blockWidth){
				double v = this.axis.java2DToValue(leftX + i, target,RectangleEdge.BOTTOM);
				Paint p = this.scale.getPaint(v);
				g2.setPaint(p);
				g2.fill(new Rectangle2D.Double(leftX + i, topY, blockWidth, this.stripWidth));
			}                
			g2.setPaint(this.stripOutlinePaint);
			g2.setStroke(this.stripOutlineStroke);
			if(stripOutlineVisible) g2.draw(outlineRect);
			this.axis.draw(g2, axisCursor, target, target, axisEdge,
					null);
		}
			...
			double topY = target.getMinY();
			int lineCount = (int)Math.rint(target.getHeight());
			//int blockHeight = (int)Math.rint(target.getHeight() / subdivisions);
			...
			if (axisEdge == RectangleEdge.LEFT) {
				leftX = target.getMaxX() - this.stripWidth;
				outlineRect = new Rectangle2D.Double(target.getMaxX()
						- this.stripWidth, target.getMinY(), this.stripWidth,
						target.getHeight());
				axisCursor = target.getMaxX() - this.stripWidth - this.axisOffset;
			}
			else if (axisEdge == RectangleEdge.RIGHT) {
				leftX = target.getMinX();
				outlineRect = new Rectangle2D.Double(target.getMinX(),
						target.getMinY(), this.stripWidth, target.getHeight());
				axisCursor = target.getMinX() + this.stripWidth
						+ this.axisOffset;
			}
			for(int i = 0; i < lineCount; i = i + blockHeight){
				double v = this.axis.java2DToValue(topY + i, target, RectangleEdge.LEFT);
				Paint p = this.scale.getPaint(v);
				g2.setPaint(p);
				g2.fill(new Rectangle2D.Double(leftX, topY + i, this.stripWidth, blockHeight));
			}                
			g2.setPaint(this.stripOutlinePaint);
			g2.setStroke(this.stripOutlineStroke);
			if(stripOutlineVisible) g2.draw(outlineRect);
			this.axis.draw(g2, axisCursor, target, target, axisEdge, null);
		}
		return null;
	}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof LinePaintScaleLegend)) {
			return false;
		}
		LinePaintScaleLegend that = (LinePaintScaleLegend) obj;
